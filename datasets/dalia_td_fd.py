...
BASE_PATH = 'clean_dalia'  
SAMPLE_RATE = 64           
VALID_METHODS = ['time', 'freq']


def evaluate_dalia(method='time', save_csv=True, save_path=None):
            ...
            for file in os.listdir(cat_path):
                ...
                if len(gt_hr) >= 3:
                    ...
                    nan_pct = check_nan_ratio([
                        estimate_hr_time_domain(d['bvp'], SAMPLE_RATE) if method == 'time'
                        else estimate_hr_frequency_domain(d['bvp'], SAMPLE_RATE)
                        for d in data
                    ])
                    results.append({
                        'Subject': subject,
    ...
    if save_csv:
        if save_path is None:
            save_path = f'dalia_{method}_domain_results.csv'
...
if __name__ == '__main__':
    evaluate_dalia(method='time')
    evaluate_dalia(method='freq')